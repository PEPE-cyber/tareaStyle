...
{   ROS_INFO("Turtle update Called  w=%f", w);
	...
	{ fx1 = pos_.x(); fy1 = pos_.y();
	  ...
	  if (nw_or < 2)
		...
		if(nw_or == 0)
		if(state == 2)  { nw_or = 3;  state = 1; }
		else if (bp) { nw_or = 1;  state = 0; }
		else state = 2;
		else if(nw_or == 1)
		if(state == 2)  { nw_or = 0;  state = 1; }
		else if (bp) { nw_or = 2;  state = 0; }
		else state = 2;
		else if(nw_or == 2)
		if(state == 2)  { nw_or = 1;  state = 1; }
		else if (bp) { nw_or = 3;  state = 0; }
		else state = 2;
		else if(nw_or == 3)
		if(state == 2)  { nw_or = 2;  state = 1; }
		else if (bp) { nw_or = 0;  state = 0; }
		else state = 2;
	 ROS_INFO("Orientation=%f  STATE=%f", nw_or, state);
     z = state == 2;
     mod = true;
